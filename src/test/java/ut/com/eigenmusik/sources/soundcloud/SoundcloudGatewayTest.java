...
import com.eigenmusik.sources.soundcloud.SoundcloudConfiguration;
import com.eigenmusik.sources.soundcloud.SoundcloudAccessToken;
import com.eigenmusik.sources.soundcloud.SoundcloudTrack;
...
public class SoundcloudGatewayTest {
    ...
    public void testGetTracks() throws IOException {
        Long soundcloudId = 123L;

        mockServer.expect(requestTo("https://api.soundcloud.com/users/" + soundcloudId + "/favorites?client_id=" + config.getClientId()))
        ...
        user.setSoundcloudId(soundcloudId);

        List<SoundcloudTrack> responseTracks = soundcloudGateway.getTracks(user);

        mockServer.verify();
        assertThat(responseTracks.get(0).getArtist(), is("Data Transmission"));
    ...
    public void testGetMe() throws IOException {
        ...
        mockServer.expect(requestTo("https://api.soundcloud.com/me?oauth_token=" + accessToken.getAccessToken()))
                .andExpect(method(HttpMethod.GET))
